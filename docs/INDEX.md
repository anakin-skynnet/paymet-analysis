...
| **Stack** | Databricks (Lakeflow, Unity Catalog, SQL Warehouse, MLflow, Model Serving, Genie), Lakebase (Postgres), Vector Search, streaming features, FastAPI + React app, ResponsesAgent (10 UC tools + python_exec) + custom agent framework (5 specialists), 3 unified dashboards, 16+ gold views, 4 ML model serving endpoints + 1 agent endpoint (`payment-response-agent`). All serverless. Closed-loop DecisionEngine with parallel ML + VS enrichment, streaming features, outcome recording, and thread-safe caching. |

...
**App resources in the bundle** (`resources/fastapi_app.yml`): SQL warehouse, Secret (scope `payment-analysis`, key `api-key` — create this scope and key in **Workspace → Secrets** so the resource binds), UC volume (reports), Jobs 1–7, and four model serving endpoints. Genie space is commented out so deploy succeeds; to show it in the portal: set `--var genie_space_id=<uuid>`, uncomment the `genie-space` block in `resources/fastapi_app.yml`, and redeploy. For **full configuration** (all resource types from the Apps UI “Add resource” list), add in **Compute → Apps → payment-analysis → Edit → Configure → App resources → + Add resource** after deploy (bundle YAML does not yet support these): **Database** (Lakebase, Can connect and create), **MLflow experiment** (Can read), **Vector search index** (Can select), **UC function** (Can execute), **UC connection** (Use connection).
